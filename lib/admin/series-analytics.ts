...
/** Query param / select value for platform-wide analytics (not a real series row). */
export const ALL_SERIES_ANALYTICS_ID = "all";

export function isAllSeriesAnalytics(seriesId: string | null | undefined): boolean {
  return seriesId === ALL_SERIES_ANALYTICS_ID;
}

const ALL_SERIES_OPTION: SeriesOption = {
  id: ALL_SERIES_ANALYTICS_ID,
  title: "All series",
  slug: "all",
  status: "published",
  total_episodes: 0,
...
export async function loadAllSeriesAnalytics(
  range: DateRange,
  options?: { sourceFilter?: TrafficSourceFilter }
  ...
  const views = tablesReady
    ? {
        value: viewsFromEvents,
        source: "events" as MetricSource,
        note: "Count of play-start events across all series in the selected range (signed-in and guest).",
      }
    : {
        value: null,
        source: "untracked" as MetricSource,
  ...
  const uniqueViewers = tablesReady && hasEventPlays
    ? {
        value: uniqueFromEvents,
        source: "events" as MetricSource,
        note: "Signed-in users only across all series. Guests are included in Total views but cannot be deduplicated without an account.",
      }
    : historyQuery.error
      ? {
          value: null,
  ...
  const paywallConversion = {
    ...
    source: tablesReady ? ("events" as MetricSource) : ("untracked" as MetricSource),
    note: tablesReady
      ? "Platform-wide paywall hits vs purchase events logged after migration 027."
      : ANALYTICS_METRIC_NOTES.paywall,
  };
  ...
  return {
    ...
    fullSeriesCompletion: {
      ...
      completers: null,
      source: "untracked" as MetricSource,
      note: "Select a single series to see full-series completion.",
    },
    paywallConversion,
    dropOff: [],
    revenue,
...
async function loadPlatformRevenue(
  ...
  if (profileIds) {
    rows = rows.filter((row) => !row.user_id || profileIds.has(row.user_id));
  }
  if (!rows.length) {
    return {
      ...untracked,
  ...
  for (const row of refunds) {
    if (row.series_id) refundDirect += row.amount_gross_cents;
    else refundUnattrib += row.amount_gross_cents;
  }

  const gross = directGross + unattribGross;
  const fees = directFee + unattribFee;
  const tax = directTax + unattribTax;
  const appStore = directApp + unattribApp;
  const refundsCents = refundDirect + refundUnattrib;
  const delivery = 0;
  const net = gross - fees - tax - appStore - refundsCents - delivery;
  const licensor = Math.round(net * LICENSOR_SHARE);
  ...
  return {
    ...
    licensorShareCents: licensor,
    directCents: directGross,
    prorataCents: unattribGross,
    note:
      "Platform-wide gross from all checkout-attributed series payments plus unattributed subscriptions. Net = gross − Stripe fees − tax − app-store (0 on web) − refunds. Delivery costs are not tracked. Licensor share line is 50% of Net Revenue (platform total, not per-licensor).",
  };
}
